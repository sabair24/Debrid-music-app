...
void main() {
  ...
  group('bonusTracks', () {
    test('what the other pressings have that yours does not', () {
      ...
      final out = bonusTracks(mine, const [], 'Backstreet Boys', [('CD · US · 1997', us), ('CD · MY · 1997', my2)]);
    ...
    test('a title you already have is never a bonus, however it is spelt', () {
      ...
      final out = bonusTracks(mine, const [], 'Backstreet Boys', [('CD · JP', other)]);
    ...
    test('nor when only YOUR PRESSING spells it differently', () {
      ...
      expect(bonusTracks(mine, const [], 'Backstreet Boys', [('Digital Media · XW', other)]), isEmpty);
    });

    test('nor when it is on disk but not on your pressing', () {
      // The radio edit exactly as it sits in the folder: the marker is in the FILENAME, not in the
      // title tag. Judged on titles it read as a track the user didn't have, and the page offered
      // them back their own file as track 17.
      final mine = [_o(1, "Everybody (Backstreet's Back)", 227)];
      final onDisk = [
        _t("Everybody (Backstreet's Back)", 227,
            path: r"D:\Flac music 2024\Albums\Backstreet Boys\Backstreet's Back\"
                "01 - Everybody (Backstreet's Back).flac"),
        _t("Everybody (Backstreet's Back)", 225,
            path: r"D:\Flac music 2024\Albums\Backstreet Boys\Backstreet's Back\"
                "01 - Everybody (Backstreet's Back) (Radio Edit).flac"),
      ];
      final other = [_o(1, "Everybody (Backstreet's Back) (radio edit)", 225)];
      expect(bonusTracks(mine, onDisk, 'Backstreet Boys', [('Digital Media · XW', other)]),
          isEmpty);
    });
    ...
    test('a genuine extra survives both checks', () {
      ...
      final out = bonusTracks(mine, onDisk, 'Backstreet Boys', [('CD · US · 1997', other)]);
    ...
    test('the same extra on two pressings is listed once, credited to the first', () {
      ...
      final out = bonusTracks(mine, const [], 'Backstreet Boys', [('CD · US', a), ('CD · AU', b)]);
